...
export const InfiniteGamesList = ({ initialGames }: InfiniteGamesListProps) => {
  ...
  const loadMoreGames = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const { data: response } = await getJackpotGames({ page });
      ...
      console.error("Failed to load more jackpot games", error);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <>
      <Games games={games} icon={<FaGem />} title={"Jackpot Games"} url="" />
