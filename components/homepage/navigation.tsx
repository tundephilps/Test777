...
export default function CasinoNavigation() {
  ...
  return (
    <div className="w-full text-white overflow-clip">
      {/* Top Navigation Bar */}
      <div className="flex flex-col items-center sm:flex-row sm:items-center sm:justify-between py-4 border-b border-gray-800 gap-3">
        ...
        <div className="flex items-center gap-2 overflow-x-auto scrollbar-hide pb-2 sm:pb-0  sm:px-0  sm:mx-0">
          <div className="flex items-center gap-2 sm:gap-3 min-w-max  sm:px-0">
            {navItems.map((item, index) => (
              <button
                key={index}
                className={`flex items-center  lg:gap-2 gap-1 px-2 lg:px-6 py-2 rounded-md text-[10px] lg:text-sm font-medium transition-all whitespace-nowrap ${
                  item.active
            ...
            {!showSearch ? (
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="bg-gray-800/50 text-gray-300 pl-14 placeholder-gray-500 rounded-md w-[80vw]   lg:pr-4  py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-600 "
                />
              </div>
            )}
          </div>
