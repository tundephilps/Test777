...
export const Games = ({ icon, title, games, url = "/slots" }: GamesProps) => {
  ...
  const handleFavoriteToggle = (gameId: number, gameName: string) => {
    const isCurrentlyFavorite = favorites.has(gameId);
    toggleFavorite(gameId, gameName, isCurrentlyFavorite);
  };
        ...
        {Array.isArray(games) &&
          games.map((game) => (
            <GameCard
              ...
              onFavoriteToggle={() =>
                handleFavoriteToggle(game.id, game.game_name)
              }
            />
          ))}
      </div>
